"""
manual_stats.py  (Argentina vs Spain -- FILLED WITH REAL DATA)
------------------------------------------------------------------
...
- Argentina reached the final with a perfect 7-0-0 record, but have
  ...
  scored a stoppage-time winner (Messi involved in both Argentina goals).
  Messi has 8 goals this tournament (level with Mbappe for the Golden
  Boot lead before the semis, though Mbappe and France have now been
  eliminated) but has gone two straight games without scoring himself
  (the Switzerland QF and England SF), while still being heavily involved
  creatively.
    ...
    "players": [
        ...
        },
        {
            ...
            "events": [
                ...
                {"date": "2026-07-15", "type": "assist"},  # assist in Enzo Fernandez equalizer vs England, SF
                {"date": "2026-07-15", "type": "assist"},  # assist in Lautaro Martinez winner vs England, SF
                {"date": "2026-07-15", "type": "rating", "value": 8.1},
            ],
        },
    ...
    "players": [
        {
            ...
            "events": [
                ...
                {"date": "2026-07-10", "type": "injury_or_out"},   # symbolic negative event: 650-min streak snapped, conceded first goal of the tournament
            ],
        },
...
HEAD_TO_HEAD = {
    "team1_win_rate": 0.43,    # Argentina: 6 wins out of 14
    "avg_goal_diff": -2.0,     # based on the two specific scorelines found (skewed by the 2018 rout, from Argentina's perspective); overall series is much closer (6-6-2)
}
...
# Real past Argentina vs Spain results for backtesting (the only two
# well-documented meetings found; team1 = Argentina, team2 = Spain)
KNOWN_PAST_RESULTS = [
    # (is_team1_home, team1_goals, team2_goals)
    (True, 2, 1),    # 1966 World Cup group stage (England, neutral-ish), Argentina won 2-1
    (False, 1, 6),   # 2018 friendly (Seville), Spain won 6-1 (Isco hat-trick)
]